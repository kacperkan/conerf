...
class TrainState:
    ...
    masking_gamma: Optional[jnp.ndarray] = None
    attribute_sheet_alpha: Optional[jnp.ndarray] = None
    ...
    def extra_params(self):
        return {
            ...
            "masking_gamma": self.masking_gamma,
            "attribute_sheet_alpha": self.attribute_sheet_alpha,
        }
...
def sample_along_rays(
    ...
    if use_stratified_sampling:
        ...
        z_vals = jnp.broadcast_to(
            z_vals[None, ...], [batch_size, num_coarse_samples]
        )
...
def volumetric_rendering(
    ...
    Args:
      ...
      z_vals: an array of size (B,S) containing the z-coordinate of the
        samples.
      ...
      key: additional key to make unique naming.

    Returns:
      A dictionary containing:
        [key_]rgb: an array of size (B,3) containing the rendered colors.
        [key_]depth: an array of size (B,) containing the rendered depth.
        [key_]acc: an array of size (B,) containing the accumulated density.
        [key_]weights: an array of size (B,S) containing the weight of each
            sample.
    """
    ...
    if sigma.ndim == 3:
        dists = dists[..., None] * jnp.linalg.norm(
            dirs[..., None, :], axis=-1, keepdims=True
        )
        alpha = 1.0 - jnp.exp(-sigma * dists)
        # Prepend a 1.0 to make this an 'exclusive' cumprod as in
        # `tf.math.cumprod`.
        accum_prod = jnp.concatenate(
            [
                jnp.ones_like(alpha[..., :1, :], alpha.dtype),
                jnp.cumprod(1.0 - alpha[..., :-1, :] + eps, axis=-2),
            ],
            axis=-2,
        )
        weights = alpha * accum_prod

        rgb = (weights * rgb).sum(axis=-2)
        exp_depth = (weights * z_vals[..., None]).sum(axis=-2)
        med_depth = compute_depth_map_attribute(weights, z_vals[..., None])
        acc = weights.sum(axis=-2)
        if use_white_background:
            rgb = rgb + (1.0 - acc)

        if sample_at_infinity:
            acc = weights[..., :-1, :].sum(axis=-2)
    else:
    ...
    prefix = f"{key}_" if key is not None and len(key) > 0 else ""
    out = {
        prefix + "rgb": rgb,
        prefix + "depth": exp_depth,
        prefix + "med_depth": med_depth,
        prefix + "acc": acc,
        prefix + "weights": weights,
    }
    return out
...
def compute_opaqueness_mask(weights, depth_threshold=0.5):
    ...
    Args:
      ...
      depth_threshold: the accumulation threshold which will be used as the
        depth termination point.

    Returns:
      A tensor containing a mask with the same size as weights that has one
        element long the sample dimension that is 1.0. This element is the
        point where the 'surface' is.
    """
...
def compute_opaqueness_mask_attribute(weights, depth_threshold=0.5):
    """Computes a mask which will be 1.0 at the depth point.
    ...
    Returns:
      A tensor containing a mask with the same size as weights that has one
        ...
        point where the 'surface' is.
    """
    cumulative_contribution = jnp.cumsum(weights, axis=-2)
    depth_threshold = jnp.array(depth_threshold, dtype=weights.dtype)
    opaqueness = cumulative_contribution >= depth_threshold
    false_padding = jnp.zeros_like(opaqueness[..., :1, :])
    padded_opaqueness = jnp.concatenate(
        [false_padding, opaqueness[..., :-1, :]], axis=-2
    )
    opaqueness_mask = jnp.logical_xor(opaqueness, padded_opaqueness)
    opaqueness_mask = opaqueness_mask.astype(weights.dtype)
    return opaqueness_mask
...
def compute_depth_map_attribute(weights, z_vals, depth_threshold=0.5):
    """Compute the depth using the median accumulation.
    ...
    Returns:
      A tensor containing the depth of each input pixel.
    """
    opaqueness_mask = compute_opaqueness_mask_attribute(
        weights, depth_threshold
    )
    return jnp.sum(opaqueness_mask * z_vals, axis=-2)
...
def noise_regularize(key, raw, noise_std, use_stratified_sampling):
    ...
    Args:
      ...
      use_stratified_sampling: add noise only if use_stratified_sampling is
        True.

    Returns:
      raw: jnp.ndarray(float32), [batch_size, num_coarse_samples, 4], updated
        raw.
    """
    if (noise_std is not None) and noise_std > 0.0 and use_stratified_sampling:
        unused_key, key = random.split(key)
        noise = (
            random.normal(key, raw["alpha"].shape, dtype=raw["alpha"].dtype)
            * noise_std
        )
...
def metadata_like(rays, metadata_id):
    ...
    return jnp.full_like(
        rays[..., :1], fill_value=metadata_id, dtype=jnp.uint32
    )
...
def vmap_module(module, in_axes=0, out_axes=0, num_batch_dims=1):
    ...
    Args:
      ...
      num_batch_dims: the number of batch dimensions (how many times to apply
        vmap to the module).
