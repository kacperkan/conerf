...
from wrapt.wrappers import AttributeWrapper

from conerf import model_utils, models, utils
...
class ScalarParams:
    ...
    mask_laplacian_weight: float = 0.1
    latent_l2_reg_weight: float = 0.0
    weight_l2_reg_weight: float = 0.0
...
def save_checkpoint(path, state, keep=2):
    ...
    logging.info(
        "Saved checkpoint: step=%d, path=%s", int(step), checkpoint_path
    )
...
def nearest_rotation_svd(matrix, eps=1e-6):
    ...
    u, _, vh = jnp.linalg.svd(
        matrix + eps, compute_uv=True, full_matrices=False
    )
...
def compute_background_loss(
    ...
    metadata = random.choice(
        key, model.warp_embeds, shape=(points.shape[0], 1)
    )
    ...
    warp_out = warp_fn(
        {"params": params}, points, metadata, state.extra_params
    )
...
@functools.partial(
    ...
    static_argnames=(
        ...
        "use_weight_l2_reg",
        "use_attribute_loss",
        "use_attribute_mask_loss",
        "use_attribute_mask_laplacian_loss",
        "use_depth_loss",
        "use_focal_loss",
        "num_attributes",
    ),
...
def train_step(
    ...
    use_latent_l2_reg: bool = False,
    use_weight_l2_reg: bool = False,
    use_attribute_loss: bool = False,
    use_attribute_mask_loss: bool = False,
    use_attribute_mask_laplacian_loss: bool = False,
    ...
    Args:
      ...
      use_attribute_loss: if True use the conditioning signal.
    ...
    def _compute_loss_and_stats(
        ...
        if "channel_set" in batch["metadata"]:
            ...
            for i in range(num_sets):
                loss = (
                    model_out["rgb"][..., i * 3 : (i + 1) * 3] - batch["rgb"]
                ) ** 2
        ...
        loss = rgb_loss
        if use_elastic_loss:
            elastic_fn = functools.partial(
                compute_elastic_loss, loss_type=elastic_loss_type
            )
        ...
        if use_warp_reg_loss:
            ...
            depth_indices = model_utils.compute_depth_index(weights)
            warp_mag = (
                (model_out["points"] - model_out["warped_points"][..., :3])
                ** 2
            ).sum(axis=-1)
            ...
            warp_reg_loss = utils.general_loss_with_squared_residual(
                ...
                scale=scalar_params.warp_reg_loss_scale,
            ).mean()
        ...
        if use_hyper_reg_loss:
            ...
            stats["residual/hyper_reg"] = jnp.mean(
                jnp.sqrt(hyper_reg_residual)
            )
        ...
        if use_attribute_mask_loss:
            are_attributes_given = batch["metadata"]["attribute_is_given"]
            if "attribute_rgb" in model_out:
                if "channel_set" in batch["metadata"]:
                    raise NotImplementedError()
                else:
                    attributes_given_mask = are_attributes_given
                    attribute_pred = jnp.clip(
                        nn.sigmoid(
                            model_out["attribute_rgb"][..., :num_attributes]
                        ),
                        1e-4,
                        1 - 1e-4,
                    )
                    ...
                    if use_focal_loss:
                        gamma = 1.5
                        alpha = 0.5
                        pos_weights = alpha * (1 - attribute_pred) ** gamma
                        neg_weights = (1 - alpha) * attribute_pred**gamma
                        ...
                        attribute_mask_loss = (
                            -(
                                (
                                    pos_weights
                                    * attribute_true
                                    * jnp.log(attribute_pred)
                                    + (1 - attribute_true)
                                    * neg_weights
                                    * jnp.log(1 - attribute_pred)
                                )
                                * attributes_given_mask
                            ).sum(axis=-1)
                        ).sum() / (attributes_given_mask.sum() + 1e-7)
                    else:
                        attribute_mask_loss = (
                            -(
                                (
                                    attribute_true * jnp.log(attribute_pred)
                                    + (1 - attribute_true)
                                    * jnp.log(1 - attribute_pred)
                                )
                                * attributes_given_mask
                ...
                stats["loss/attribute_mask_loss"] = attribute_mask_loss
                loss += scalar_params.mask_loss_weight * attribute_mask_loss

        if use_attribute_loss:
            are_attributes_given = batch["metadata"]["attribute_is_given"]
            are_attributes_given = jnp.repeat(
                are_attributes_given[:, None],
                repeats=model_out["warped_points"].shape[1],
                axis=1,
            )
            attributes_gt_values = batch["metadata"]["attribute_values"]
            attribute_loss = (
                (
                    (model_out["hyper_projected_embed"] - attributes_gt_values)
                    ** 2
                )
                * batch["metadata"]["attribute_is_given"]
            ).sum() / (batch["metadata"]["attribute_is_given"].sum() + 1e-7)
            stats["loss/attribute_loss"] = (
                scalar_params.attribute_loss_weight * attribute_loss
            )
            loss += scalar_params.attribute_loss_weight * attribute_loss

        if use_latent_l2_reg:
            embed = None
            if "warp_embed" in model_out:
                embed = model_out["warp_embed"]
            if "hyper_embed" in model_out:
                embed = (
                    jnp.concatenate((embed, model_out["hyper_embed"]), axis=-1)
                    if embed is not None
                    else model_out["hyper_embed"]
                )
            if embed is None:
                raise ValueError(
                    "Can't use l2 regularization when no embedding is "
                    "available"
                )
            latent_reg = (embed**2).sum(axis=-1).mean()
            stats["loss/latent_reg"] = (
                scalar_params.latent_l2_reg_weight * latent_reg
            )
            loss += scalar_params.latent_l2_reg_weight * latent_reg
        if use_weight_l2_reg:

            def _reg(pytree):
                return jax.tree_util.tree_reduce(
                    lambda acc, tensor: acc + (tensor**2).sum(), pytree, 0.0
                )

            weight_reg = _reg(params["model"]["attribute_sheet_mlp"]).mean()

            stats["loss/weight_reg"] = (
                scalar_params.weight_l2_reg_weight * weight_reg
            )
            loss += scalar_params.weight_l2_reg_weight * weight_reg
        if use_attribute_mask_laplacian_loss:
            assert "noised_attribute_3d" in model_out
            laplacian = (
                (model_out["attribute_3d"] - model_out["noised_attribute_3d"])
                ** 2
            ).mean()

            stats["loss/mask_lapl"] = (
                scalar_params.mask_laplacian_weight * laplacian
            )
            loss += scalar_params.mask_laplacian_weight * laplacian
        if use_depth_loss:
            depth_loss = (
                (model_out["depth"] - batch["depth"][..., 0]) ** 2
            ).mean()

            stats["loss/depth_loss"] = (
                scalar_params.depth_loss_weight * depth_loss
            )
            loss += scalar_params.depth_loss_weight * depth_loss
    ...
    def _loss_fn(params):
        ret = model.apply(
            ...
            return_points=(
                use_warp_reg_loss or use_hyper_reg_loss or use_attribute_loss
            ),
            return_weights=(
                use_warp_reg_loss or use_elastic_loss or use_weight_l2_reg
            ),
        ...
        if "fine" in ret:
            losses["fine"], stats["fine"] = _compute_loss_and_stats(
                ...
                ret["fine"],
                "fine",
                use_attribute_loss=use_attribute_loss,
            )
        if "coarse" in ret:
    ...
    if disable_hyper_grads:
        optimizer = optimizer.replace(
            state=zero_adam_param_states(
                optimizer.state, "model/hyper_sheet_mlp"
            )
        )
