...
from conerf import model_utils, modules
from conerf import rigid_body as rigid
from conerf import types, utils
...
class TranslationField(nn.Module):
    ...
    def warp(
        ...
        points_embed = model_utils.posenc(
            points,
    ...
    def __call__(
        ...
        Returns:
          The warped points and the Jacobian of the warp if `return_jacobian` is
            True.
        """
        out = {"warped_points": self.warp(points, metadata, extra_params)}
...
class SE3Field(nn.Module):
    ...
    translation_init: types.Initializer = jax.nn.initializers.uniform(
        scale=1e-4
    )
