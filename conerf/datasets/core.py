...
from conerf import camera as cam
from conerf import gpath, image_utils
from conerf import tf_camera as tfcam
from conerf import utils
...
def camera_to_rays(camera: cam.Camera):
    ...
    img_rays_origin = np.tile(
        camera.position[None, None, :], image_shape + (1,)
    )
...
def load_camera(
    ...
    Args:
      ...
      scene_center: the center of the scene where the camera will be centered
        to.
      scene_scale: the scale of the scene by which the camera will also be
        scaled by.
...
def prepare_zip_tf_data(xs, ys, devices=None):
    """Convert a input batch from tf Tensors to numpy arrays."""
    if devices is None:
        devices = jax.local_devices()
    ...
    return jax.tree_map(_prepare, xs, ys)
...
def prepare_zip_tf_data_unbatched(xs, ys):
    """Prepare TF dataset into unbatched numpy arrays."""
    # Use _numpy() for zero-copy conversion between TF and NumPy.
    # pylint: disable=protected-access
    return jax.tree_map(
        lambda x, y: np.concatenate((x._numpy(), y._numpy()), axis=0), xs, ys
    )


def iterator_from_dataset(
    dataset: Union[tf.data.Dataset, Tuple[tf.data.Dataset, tf.data.Dataset]],
    batch_size: int,
    repeat: bool = True,
    prefetch_size: int = 0,
    devices: Optional[Sequence[Any]] = None,
    samples_with_attribute_ratio: float = 0.0,
):
    ...
    if isinstance(dataset, tuple):
        with_attribute_batch_size = int(
            batch_size * samples_with_attribute_ratio
        )
        wo_attribute_batch_size = batch_size - with_attribute_batch_size
        dataset_with_attr, dataset_wo_attr = dataset

        if repeat:
            dataset_with_attr = dataset_with_attr.repeat()
            dataset_wo_attr = dataset_wo_attr.repeat()
        if batch_size > 0:
            dataset_with_attr = dataset_with_attr.batch(
                with_attribute_batch_size
            )
            dataset_wo_attr = dataset_wo_attr.batch(wo_attribute_batch_size)
            it = map(
                functools.partial(prepare_zip_tf_data, devices=devices),
                dataset_with_attr,
                dataset_wo_attr,
            )
        else:
            it = map(
                prepare_zip_tf_data_unbatched,
                dataset_with_attr,
                dataset_wo_attr,
            )
    else:
        ...
        if batch_size > 0:
            ...
            it = map(
                functools.partial(prepare_tf_data, devices=devices), dataset
            )
        else:
...
def _camera_to_rays_fn(item, use_tf_camera=False):
    ...
    if use_tf_camera:
        ...
        origins = tf.broadcast_to(
            camera.position[None, None, :], tf.shape(directions)
        )
    else:
        ...
        origins = np.broadcast_to(
            camera.position[None, None, :], directions.shape
        )
...
class DataSource(abc.ABC):
    ...
    @abc.abstractmethod
    def load_attribute_mask(self, item_id, height: int, width: int):
        raise NotImplementedError()

    @abc.abstractmethod
    def load_attribute_is_given(self, item_id):
        raise NotImplementedError()

    @abc.abstractmethod
    def load_attribute_values(self, item_id):
        raise NotImplementedError()
    ...
    def create_cameras_dataset(
        self,
        cameras: Union[Sequence[cam.Camera], Sequence[gpath.GPath]],
        flatten=False,
        shuffle=False,
    ):
        ...
        dataset = dataset.map(
            functools.partial(_camera_to_rays_fn, use_tf_camera=True),
            _TF_AUTOTUNE,
        )
    ...
    def create_iterator(
        ...
        dataset = self.create_dataset(
            ...
            samples_with_attribute_ratio=samples_with_attribute_ratio,
        )
    ...
    def create_dataset(
        ...
        samples_with_attribute_ratio=0.0,
    ):
    ...
    def _create_preloaded_dataset(
        ...
        def _process_list(data_list):
            ...
            def _prepare_array(x):
                ...
                # The `and` part of the check ensures we're not touching ragged
                # arrays.
                if x.ndim == 1 and x[0].ndim == 0:
                    x = np.expand_dims(x, -1)
                if flatten:
                    x = np.concatenate(
                        [x.reshape(-1, x.shape[-1]) for x in x], axis=0
                    )
                if shuffle:
                    x = x[shuffled_inds]
                return x
        ...
        if not is_valid and samples_with_attribute_ratio > 0:
            data_list_with_attributes = [
                item
                for item in data_list
                if np.any(item["metadata"]["attribute_is_given"] > 0)
            ]
            data_list_without_attributes = [
                item
                for item in data_list
                if np.all(item["metadata"]["attribute_is_given"] == 0)
            ]

            return tf.data.Dataset.from_tensor_slices(
                _process_list(data_list_with_attributes)
            ), tf.data.Dataset.from_tensor_slices(
                _process_list(data_list_without_attributes)
            )

        return tf.data.Dataset.from_tensor_slices(_process_list(data_list))

    def _create_lazy_dataset(
        self,
        item_ids,
        flatten=False,
        shuffle=False,
        ...
        dataset = dataset.map(
            _camera_to_rays_fn, num_parallel_calls=_TF_AUTOTUNE
        )
    ...
    def _get_item_signature(self):
        ...
        sig = {
            ...
            "attribute_rgb": tf.TensorSpec(
                shape=(None, None, self.num_attributes + 1), dtype=tf.float32
            ),
        ...
        if self.num_attributes > 0:
            metadata = {
                "attribute_is_given": tf.TensorSpec(
                    shape=(
                        None,
                        self.num_attributes,
                    ),
        ...
        if self.num_attributes > 0:
            sig["attribute_rgb"] = tf.TensorSpec(
                shape=(None, None, self.num_attributes + 1), dtype=tf.float32
            )
        ...
        if self.use_camera_id:
            sig["metadata"]["camera"] = tf.TensorSpec(
                shape=[None], dtype=tf.uint32
            )
        if self.use_warp_id:
            sig["metadata"]["warp"] = tf.TensorSpec(
                shape=[None], dtype=tf.uint32
            )
        if self.use_time:
            sig["metadata"]["time"] = tf.TensorSpec(
    ...
    def _get_item_py_function(self, item_id):
        ...
        The only way to get Tensorflow to evaluate our simple `get_item`
        function in parallel seems to be to wrap it using tf.py_function or
        tf.numpy_function.
        ...
        flat_values = tf.numpy_function(
            _get_flat_item, [item_id], flattened_types
        )
    ...
    def get_item(self, item_id, scale_factor=1.0):
        ...
        Returns:
          A dictionary containing one of more of the following items:
            ...
            `metadata`: a dictionary of containing various metadata arrays.
              Each item is an array containing metadata IDs for each ray.
        """
        rgb = self.load_rgb(item_id)
        attribute = self.load_attribute_mask(
            item_id, rgb.shape[0], rgb.shape[1]
        )
        if scale_factor != 1.0:
            rgb = image_utils.rescale_image(rgb, scale_factor)
            attribute = image_utils.rescale_image(
                attribute, scale_factor, cv2.INTER_NEAREAST
            )

        depth = self.load_depth(item_id)[..., np.newaxis]
        camera = self.load_camera(item_id, scale_factor)
        if self.num_attributes > 0:
            attribute_is_given = self.load_attribute_is_given(item_id)
            attribute_values = self.load_attribute_values(item_id)
            metadata = {
                "attribute_is_given": attribute_is_given,
        ...
        if self.use_camera_id:
            data["metadata"]["camera"] = np.atleast_1d(
                self.get_camera_id(item_id)
            )
        if self.use_warp_id:
