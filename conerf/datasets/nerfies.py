...
class NerfiesDataSource(core.DataSource):
    ...
    def load_attribute_is_given(self, item_id: str) -> np.ndarray:
        if int(item_id) in self.annotations:
            return np.ones((self.num_attributes,), dtype=np.float32)
        return np.zeros((self.num_attributes,), dtype=np.float32)
    ...
    def load_camera(
        self, item_id: types.PathType, scale_factor: float = 1.0
    ) -> cam.Camera:
