...
def fit_lr(gt_attributes, gt_betas):
    params = jnp.linalg.inv(gt_attributes.T @ gt_attributes) @ (
        gt_attributes.T @ gt_betas
    )
    return params


def encode_attribute(params, attribute_set, key):
    encoded_metadata = {key: attribute_set @ params}
    return encoded_metadata


def encode_lr_data(datasource, model, state, batch):
    ...
    del batch["metadata"]["warp"]
    del batch["metadata"]["hyper_embed"]
    return batch
...
def render_image(
    ...
    rays_dict = tree_util.tree_map(
        lambda x: x.reshape((num_rays, -1)), rays_dict
    )
    ...
    logging.info(
        "Rendering: num_batches = %d, num_rays = %d, chunk = %d",
        num_batches,
        num_rays,
        chunk,
    )
    for batch_idx in range(num_batches):
        ...
        logging.debug(
            "Rendering batch: num_chunk_rays = %d, padding = %d, "
            "remainder = %d, "
            "per_proc_rays = %d",
            num_chunk_rays,
            padding,
        ...
        chunk_rays_dict = tree_util.tree_map(
            lambda x: x[
                (proc_id * per_proc_rays) : ((proc_id + 1) * per_proc_rays)
            ],
            chunk_rays_dict,
        )
        ...
        if ret_keys is not None:
            for key in list(ret_map.keys()):
                if key not in ret_keys:
                    del ret_map[key]
        ret_maps.append(ret_map)
    ret_map = jax.tree_multimap(
        lambda *x: jnp.concatenate(x, axis=0), *ret_maps
    )
    logging.info("Rendering took %.04s", time.time() - start_time)
    out = {}
    for key, value in ret_map.items():
        if (ret_keys is not None and key not in ret_keys) or value is None:
            continue

        out_shape = (*batch_shape, *value.shape[1:])
        logging.debug(
            "Reshaping %s of shape %s to %s",
            key,
            str(value.shape),
            str(out_shape),
        )
