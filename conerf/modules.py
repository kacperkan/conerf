...
class HyperSheetMLP(nn.Module):
    ...
    def _encode_embed(self, points, embed, decorrelate):
        if decorrelate:
            outputs = []
    ...
    @nn.compact
    def __call__(self, points, embed, main_embed, alpha=None):
        ...
        if self.use_main_embed:
            outputs = jnp.concatenate(
                (
                    self._encode_embed(
                        points_feat, embed, self.decorrelate_hyper_dims
                    ),
