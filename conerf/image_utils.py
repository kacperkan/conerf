...
from conerf import gpath, types
...
def downsample_image(
    image: np.ndarray, scale: int, interp: int = cv2.INTER_AREA
) -> np.ndarray:
    ...
    resized = cv2.resize(image, (out_width, out_height), interp)
    return resized
...
def reshape_image(
    image: np.ndarray, shape: Tuple[int, int], interp: int = cv2.INTER_AREA
) -> np.ndarray:
    """Reshapes the image to the given shape."""
    out_height, out_width = shape
    return cv2.resize(image, (out_width, out_height), interpolation=interp)


def rescale_image(
    image: np.ndarray, scale_factor: float, interp: int = cv2.INTER_AREA
) -> np.ndarray:
    """Resize an image by a scale factor, using int resizing if possible."""
    ...
    if scale_factor.is_integer():
        return upsample_image(image, int(scale_factor), interp=interp)
    ...
    if (
        ...
        return downsample_image(image, int(inv_scale), interp=interp)
    ...
    return reshape_image(image, (out_height, out_width), interp=interp)
...
def checkerboard(h, w, size=8, true_val=1.0, false_val=0.0):
    ...
    pattern = np.kron([[1, 0] * j, [0, 1] * j] * i, np.ones((size, size)))[
        :h, :w
    ]
...
def make_grid(batch, grid_height=None, zoom=1, old_buffer=None, border_size=1):
    ...
    Args:
      ...
      old_buffer: Buffer to write grid into if possible. If not set, or if
        shape doesn't match, we create a new buffer.
    ...
    # We create the numpy buffer if we don't have an old buffer or if the size
    # has changed.
    shape = (
        height * grid_height + border_size * (grid_height - 1),
