...
def general_loss_with_squared_residual(x_sq, alpha, scale):
    ...
    Args:
      ...
      alpha: The shape parameter of the loss (\alpha in the paper), where more
        ...
        alpha is 0 at -infinity, infinity, 0, and 2. Varying alpha allows for
        smooth
      ...
      scale: The scale parameter of the loss. When |x| < scale, the loss is an
        L2-like quadratic bowl, and when |x| > scale the loss function takes
        on a different shape according to alpha.
...
def shard(xs, device_count=None):
    """Split data into shards to multiple devices along the first dimension."""
