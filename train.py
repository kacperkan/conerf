...
def _log_to_tensorboard(
    ...
    _log_scalar(
        "params/elastic_loss/weight", scalar_params.elastic_loss_weight
    )
    _log_scalar("params/masking_gamma", state.masking_gamma)
    _log_scalar("params/attribute_sheet_alpha", state.attribute_sheet_alpha)
...
def _log_histograms(
    writer: tensorboard.SummaryWriter, state: model_utils.TrainState, model_out
):
    ...
    for branch in {"coarse", "fine"}:
        if "warped_points" in model_out[branch]:
            ...
            writer.histogram(
                f"{branch}/spatial_points", warped_points[..., :3], step
            )
            writer.histogram(
                f"{branch}/spatial_points_delta",
                warped_points[..., :3] - points,
                step,
            )
            if warped_points.shape[-1] > 3:
                writer.histogram(
                    f"{branch}/hyper_points", warped_points[..., 3:], step
                )
...
def main(argv):
    ...
    train_iter = datasource.create_iterator(
        ...
        devices=devices,
        is_valid=False,
        samples_with_attribute_ratio=train_config.samples_with_attribute_ratio,
    )
    ...
    scalar_params = training.ScalarParams(
        ...
        attribute_loss_weight=train_config.attribute_loss_weight,
        mask_loss_weight=train_config.mask_loss_weight,
        mask_laplacian_weight=train_config.mask_laplacian_weight,
        latent_l2_reg_weight=train_config.latent_l2_reg_weight,
        weight_l2_reg_weight=train_config.weight_l2_reg_weight,
        depth_loss_weight=train_config.depth_loss_weight,
    )
    ...
    if jax.process_index() == 0:
        ...
        summary_writer.text(
            "gin/train", textdata=gin.markdown(config_str), step=0
        )
    ...
    train_step = functools.partial(
        ...
        use_attribute_loss=train_config.use_attribute_loss,
        use_attribute_mask_loss=train_config.use_attribute_mask_loss,
        use_attribute_mask_laplacian_loss=(
            train_config.use_attribute_mask_laplacian_loss
        ),
        use_latent_l2_reg=train_config.use_latent_l2_reg,
        use_weight_l2_reg=train_config.use_weight_l2_reg,
        use_depth_loss=train_config.use_depth_loss,
    ...
    for step, batch in zip(
        range(init_step, train_config.max_steps + 1), train_iter
    ):
        ...
        attribute_sheet_alpha = jax_utils.replicate(
            attribute_sheet_alpha_sched(step), devices
        )
        masking_gamma = jax_utils.replicate(masking_gamma_sched(step), devices)
        state = state.replace(
            nerf_alpha=nerf_alpha,
            warp_alpha=warp_alpha,
            hyper_alpha=hyper_alpha,
            hyper_sheet_alpha=hyper_sheet_alpha,
            masking_gamma=masking_gamma,
            attribute_sheet_alpha=attribute_sheet_alpha,
        )
        ...
        if step % train_config.print_every == 0 and jax.process_index() == 0:
            logging.info(
                "step=%d, nerf_alpha=%.04f, warp_alpha=%.04f, %s",
                step,
        ...
        if (
            step % train_config.histogram_every == 0
            and jax.process_index() == 0
        ):
            _log_histograms(
                summary_writer, jax_utils.unreplicate(state), model_out
            )
